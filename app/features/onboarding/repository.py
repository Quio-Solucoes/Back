...
from app.features.users.repository import get_user_by_empresa_and_role, user_email_exists
from app.features.users.schema import User


def owner_email_exists(db: Session, normalized_email: str) -> bool:
    return user_email_exists(db, normalized_email)


def get_owner_by_empresa_id(db: Session, empresa_id: str) -> User | None:
    return get_user_by_empresa_and_role(db, empresa_id, UserRole.OWNER)
