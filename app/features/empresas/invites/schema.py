...
class FranchiseInvite(Base):
    ...
    @staticmethod
    def build_expiration() -> datetime:
        # Convites expiram em 24 horas a partir da criação
...
class UserInvite(Base):
    ...
    email: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
    role: Mapped[MembershipRole] = mapped_column(Enum(MembershipRole, name="user_invite_role"), nullable=False)
    token_hash: Mapped[str] = mapped_column(String(64), unique=True, nullable=False, index=True)
